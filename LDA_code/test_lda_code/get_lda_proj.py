...
def lda_numeric_derivative(R,R0,precision0,v,delta):
    ...
    for atom in range(n_atoms):
        for j in range(3):
            delta_vec = np.zeros(R.shape)
            delta_vec[atom,j] = delta
            dR[atom,j] = E_lda(R+delta_vec,R0,precision0,v)
...
#vec_ = np.loadtxt("lda_scalings_global_6_states.txt")
vec_ = np.loadtxt("lda_scalings_global_folded_unfolded_ld1.txt")
...
X = np.array([traj.time, lda_projections]).T
np.savetxt("lda_projections.txt", X)
"""
st = time.time()
