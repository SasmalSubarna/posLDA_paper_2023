...
# load
grid, fe = np.loadtxt("fe_ld1_state.dat", usecols=(0,1), unpack=True)
grid *= -1
fe /= 4.184 # in kcal/mol unit

# load DESRES data (used same params, bins=100, range=[-5,15])
grid_ref, fe_ref = np.loadtxt("fe_vs_ld1_2_states_global_alignment_bins_100_range-15,5.txt", usecols=(0,1), unpack=True)
grid_ref *= -1

plt.figure(1, figsize=(7,6), dpi=80)
plt.title("2-State LD1", fontsize=24)
...
plt.plot(grid+0.5, fe, label="FE", lw=3.0, color="tab:red")
plt.legend(ncol=1, loc="upper right", fontsize="x-large")
#plt.savefig("fe_vs_ld1.pdf", format="pdf")
plt.savefig("fe_vs_ld1_rev.pdf", format="pdf")
plt.show()
